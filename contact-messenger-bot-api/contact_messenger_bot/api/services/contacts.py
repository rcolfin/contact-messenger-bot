...
class Contacts:
    ...
    def _get_profile(self) -> models.Profile:
        ...
        mobile_number = next(filter(lambda x: x.is_primary, self._get_mobile_numbers(profile)), None)
        email_address = next(filter(lambda x: x.is_primary, self._get_home_email_addresses(profile)), None)
    ...
    def _get_contacts(self) -> Iterable[models.Contact]:
        ...
        for contact in contacts:
            ...
            home_email_addresses = self._get_home_email_addresses(contact)
    ...
    def _get_mobile_numbers(self, contact: dict[str, Any]) -> list[models.PhoneNumber]:
        ...
        for number in contact.get("phoneNumbers", []):
            if number.get("type", "").casefold() != "mobile":
                continue
    ...
    def _get_home_addresses(self, contact: dict[str, Any]) -> list[models.Address]:
        return [
            ...
            if address.get("postalCode") and address.get("type", "").casefold() == "home"
        ]

    def _get_home_email_addresses(self, contact: dict[str, Any]) -> list[models.EmailAddress]:
        addresses: list[models.EmailAddress] = []
        for email_addresses in contact.get("emailAddresses", []):
            if email_addresses.get("type", "").casefold() != "home":
                continue

            primary = self._is_primary(email_addresses)
            address = email_addresses.get("value")
            addresses.append(models.EmailAddress(address, primary))
